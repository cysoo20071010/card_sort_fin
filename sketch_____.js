...
function setup() {
  ...
  // 카드 크기와 간격을 화면 크기에 비례하여 설정
  cardWidth = windowWidth / 10;
  cardHeight = windowHeight / 7;
  spacing = min(windowWidth, windowHeight) / 40;
...
function draw() {
  ...
  if (!gameStarted) {
    ...
    if (isSorted) { // 정렬 확인
      ...
      endTime = millis();
      fill(0);
      textSize(min(windowWidth, windowHeight) / 25);
